...
void module_pus_surface_float(void)
{
    ...
    while (run)
    {
        ...
        while (piston_move)
        {
            ...
            ARTEMIS_DEBUG_PRINTF("PUS :: surface_float, Piston task->state = %s\n", piston_task_running ? "running" : "not running");
    ...
    // Completely stop the piston task before moving to full length
    PIS_task_delete(); // Signal to exit loop
    vTaskDelay(xDelay5000ms); // Wait for the task to exit the loop and delete itself
    PIS_Reset();
...
void module_pds_idle(void)
{
    ...
    {
        piston_task_running = PIS_taskStatus(); // check if the piston task is running
        ARTEMIS_DEBUG_PRINTF("PDS :: Idle, Piston task->state = %s\n", piston_task_running ? "running" : "not running");
        if ( piston_task_running )
        {
    ...
    while (run)
    {
        ...
        if ( Depth>BALLAST_DEPTH )
        {
            if (piston_move)
            {
                ...
                {
                    ARTEMIS_DEBUG_PRINTF("SPS :: idle, Previous piston task is still running...\n");
                    vTaskDelay(xDelay5000ms);
                    previousTaskAlive = PIS_taskStatus(); // check if the piston task is running
                }
                vTaskDelay(piston_period);
...
void module_sps_move_to_park(void)
{
    ...
    if( pistonzero_number >= PISTON_ZEROCAL_COUNTER )
    {
        ...
        {
            piston_task_running = PIS_taskStatus(); // check if the piston task is running
            ARTEMIS_DEBUG_PRINTF("SPS :: move_to_park, Piston zero task->state = %s\n", piston_task_running ? "running" : "not running");
            if ( piston_task_running )
            {   
    ...
    // Completely stop the piston task before moving to park length
    PIS_task_delete(); // Signal to exit loop
    vTaskDelay(xDelay5000ms); // Wait for the task to exit the loop and delete itself
    PIS_Reset();
    vTaskDelay(xDelay10000ms);
    ...
    {
        piston_task_running = PIS_taskStatus(); // check if the piston task is running
        ARTEMIS_DEBUG_PRINTF("SPS :: move_to_park, Piston task->state = %s\n", piston_task_running ? "running" : "not running");
        if ( piston_task_running )
        {
...
void module_sps_move_to_profile(void)
{
    ...
    {
        piston_task_running = PIS_taskStatus(); // check if the piston task is running
        ARTEMIS_DEBUG_PRINTF("SPS :: move_to_profile, Piston task->state = %s\n", piston_task_running ? "running" : "not running");
        if ( piston_task_running )
        {
...
{   
    ...
    else
    {
        ...
        {
            piston_task_running = PIS_taskStatus(); // check if the piston task is running
            ARTEMIS_DEBUG_PRINTF("SPS :: move_to_surface B, Piston task->state = %s\n", piston_task_running ? "running" : "not running");
            if ( piston_task_running )
            {
...
void module_sps_tx(void)
{
    ...
    if (!iridium_init) {
        ...
        i9603n_initialize_power(); // initializes the supercap pins.
    ...
    while (MEM_queue_get_count() > 0 && !fatal_error_occurred && !stop_processing_queue_this_cycle)
    {
        ...
        while (!item_fully_transmitted && !fatal_error_occurred && !exit_paging_loop)
        {
            ...
            while (retry_page_internally && !fatal_error_occurred)
            {
                ...
                i9603n_off(); // Power off the modem
