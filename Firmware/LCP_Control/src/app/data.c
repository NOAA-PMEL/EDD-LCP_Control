...
void create_header_irid(uint8_t *df, pData *P, sData *S)
{
    ...
    int32_t latitude = module_convert_latitude_to_int32_t(P->pLatitude);
    buf[5] = latitude>>24;
    buf[6] = latitude>>16;
    buf[7] = latitude>>8;
    buf[8] = latitude&0xFF;
    ...
    int32_t longitude = module_convert_longitude_to_int32_t(P->pLongitude);
    buf[9]  = longitude>>24;
    buf[10] = longitude>>16;
    buf[11] = longitude>>8;
    ...
    buf[20] = (S->modeType<<4&0xF0) | (S->pageNumber&0xF);

    /* 12. Start time for one profile */
    //uint32_t start = P[profNumber].pStart;
    uint32_t start = P->pStart;
    buf[21] = start>>24;
    buf[22] = start>>16;
    buf[23] = start>>8;
    buf[24] = start&0xFF;
...
void create_header_irid_ext(uint8_t *df, pData *P, sData *S)
{
    ...
    buf[2] = (S->modeType<<4&0xF0) | (S->pageNumber&0xF);
